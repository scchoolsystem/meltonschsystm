...
CREATE POLICY "admission staff manage student docs" ON public.student_documents
  ...
  USING (public.is_admin(auth.uid()) OR public.has_role(auth.uid(),'admission_officer'::public.app_role) OR public.has_role(auth.uid(),'deputy_principal'::public.app_role))
  WITH CHECK (public.is_admin(auth.uid()) OR public.has_role(auth.uid(),'admission_officer'::public.app_role) OR public.has_role(auth.uid(),'deputy_principal'::public.app_role));
...
CREATE POLICY "tenant_isolation" ON public.student_documents AS RESTRICTIVE FOR ALL TO authenticated
...
CREATE TRIGGER trg_stamp_school_id BEFORE INSERT ON public.student_documents FOR EACH ROW EXECUTE FUNCTION public.stamp_school_id();
DROP TRIGGER IF EXISTS trg_guard_school_id ON public.student_documents;
CREATE TRIGGER trg_guard_school_id BEFORE UPDATE ON public.student_documents FOR EACH ROW EXECUTE FUNCTION public.guard_school_id();
...
CREATE POLICY "admission staff read student-documents" ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'student-documents' AND (public.is_admin(auth.uid()) OR public.has_role(auth.uid(),'admission_officer'::public.app_role) OR public.has_role(auth.uid(),'deputy_principal'::public.app_role)));
DROP POLICY IF EXISTS "admission staff write student-documents" ON storage.objects;
CREATE POLICY "admission staff write student-documents" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'student-documents' AND (public.is_admin(auth.uid()) OR public.has_role(auth.uid(),'admission_officer'::public.app_role) OR public.has_role(auth.uid(),'deputy_principal'::public.app_role)));
DROP POLICY IF EXISTS "admission staff delete student-documents" ON storage.objects;
CREATE POLICY "admission staff delete student-documents" ON storage.objects FOR DELETE TO authenticated
  USING (bucket_id = 'student-documents' AND (public.is_admin(auth.uid()) OR public.has_role(auth.uid(),'admission_officer'::public.app_role) OR public.has_role(auth.uid(),'deputy_principal'::public.app_role)));
...
CREATE UNIQUE INDEX IF NOT EXISTS uniq_insurance_default_per_school ON public.insurance_policies(school_id) WHERE is_default;
ALTER TABLE public.insurance_policies ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "admin manage insurance" ON public.insurance_policies;
CREATE POLICY "admin manage insurance" ON public.insurance_policies FOR ALL TO authenticated USING (public.is_admin(auth.uid())) WITH CHECK (public.is_admin(auth.uid()));
DROP POLICY IF EXISTS "auth view insurance" ON public.insurance_policies;
CREATE POLICY "auth view insurance" ON public.insurance_policies FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "admin manage student insurance" ON public.student_insurance FOR ALL TO authenticated USING (public.is_admin(auth.uid())) WITH CHECK (public.is_admin(auth.uid()));
DROP POLICY IF EXISTS "parent view child insurance" ON public.student_insurance;
CREATE POLICY "parent view child insurance" ON public.student_insurance FOR SELECT TO authenticated USING (public.is_parent_of(student_id));
DROP POLICY IF EXISTS "student view own insurance" ON public.student_insurance;
CREATE POLICY "student view own insurance" ON public.student_insurance FOR SELECT TO authenticated USING (public.is_student(student_id));
...
$func$
  WITH candidates AS (
    SELECT c.id, c.capacity,
           (SELECT count(*) FROM public.students s WHERE s.class_id = c.id AND s.status = 'active') AS load
    FROM public.classes c
    WHERE c.school_id = public.current_user_school()
      AND lower(c.name) = lower(_level)
      AND c.year = EXTRACT(year FROM now())::int
  )
  SELECT id FROM candidates WHERE load < capacity ORDER BY load ASC, random() LIMIT 1;
$func$;
...
$func$
  WITH candidates AS (
    SELECT d.id, d.capacity,
           (SELECT count(*) FROM public.dorm_assignments a WHERE a.dormitory_id = d.id) AS load
