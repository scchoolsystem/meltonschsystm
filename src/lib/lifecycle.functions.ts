...
export const setStaffLifecycle = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!callerSchool) throw new Error("No school context for current user");
    const { data: cur } = await supabaseAdmin
      .from("staff").select("id, user_id, lifecycle_status, school_id")
      .eq("id", data.staff_id).eq("school_id", callerSchool).maybeSingle();
    if (!cur) throw new Error("Staff not found in your school");
    const { error } = await supabaseAdmin.from("staff").update({
      ...
      transferred_to: data.to_status === "transferred" ? (data.transferred_to ?? null) : null,
      status: data.to_status === "archived" ? "archived" : "active",
    }).eq("id", data.staff_id).eq("school_id", callerSchool);
    if (error) throw new Error(error.message);
