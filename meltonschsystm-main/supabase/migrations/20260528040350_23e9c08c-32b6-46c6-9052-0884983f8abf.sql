...
-- =========================================================================
-- 7. FK on staff.department_id (SET NULL on delete)
-- =========================================================================
DO $$ BEGIN
...
-- =========================================================================
-- 8. Backfill staff_category for existing rows
-- =========================================================================
UPDATE public.staff SET staff_category = CASE
  ...
  WHEN role::text IN ('librarian','library_admin','library_user','nurse','clinic_admin','clinic_user',
                      'matron','boarding_admin','boarding_user','boarding','kitchen_admin','kitchen_user',
                      'security_admin','security_user','transport_admin','transport_officer',
                      'store_admin','store_user','sports','sports_admin','sports_user') THEN 'support'
  ELSE 'support'
END
WHERE staff_category IS NULL;

-- =========================================================================
-- 9. Seed hierarchy per existing school (idempotent)
-- =========================================================================
DO $seed$
DECLARE
  s record;
...
BEGIN
  FOR s IN SELECT id FROM public.schools LOOP
    ...
    -- Default sports activities under Sports department
