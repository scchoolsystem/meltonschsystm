...
function Page() {
  ...
  // Fetched directly (not derived from the capped `loans` list above) so an
  // old overdue loan can't silently disappear once a school has more than
  // 500 total loan records — the general list is ordered by most-recently
  // borrowed, so an old-but-still-overdue loan can fall outside that window
  // while still being live. This dashboard-parity query has no such cap.
  const { data: overdueLoans = [] } = useQuery({
    ...
    queryFn: async () =>
      (await supabase
        ...
        .eq("status", "active")
        .lt("due_on", today)
        .order("due_on", { ascending: true })).data ?? [],
  });
  ...
  const notifyMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["library-loans"] }); qc.invalidateQueries({ queryKey: ["library-loans-overdue"] }); toast.success("Reminder marked as sent"); },
  ...
  const bulkNotifyMutation = useMutation({
    ...
    onSuccess: (count) => { qc.invalidateQueries({ queryKey: ["library-loans"] }); qc.invalidateQueries({ queryKey: ["library-loans-overdue"] }); toast.success(`${count} overdue reminders marked as sent`); },
    onError: (e: any) => toast.error(e.message),
  });
