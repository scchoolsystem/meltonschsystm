                      ...
                      <div className="space-y-1">
                        {blocks.map((block) => {
                          if (!block.isElective) {
                            ...
                            return (
                              <div key={s.id} className="flex justify-between border-b py-1.5 text-sm gap-2">
                                <span className="font-mono text-xs text-muted-foreground w-24 shrink-0">{s.start_time?.slice(0, 5)}–{s.end_time?.slice(0, 5)}</span>
                                <span className="flex-1 font-medium">{s.subjects?.name}</span>
                                <span className="text-xs text-muted-foreground">{staffName(s.staff)}</span>
                                <span className="text-xs text-muted-foreground w-20 text-right shrink-0">{s.room ?? ""}</span>
                              </div>
                            );
                          }
                          return (
                            <div key={`${block.day_of_week}-${block.start_time}`} className="border rounded py-1.5 px-2">
