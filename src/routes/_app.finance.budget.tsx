...
import { upsertBudget, getBudgetVsActual } from "@/lib/finance-extended.functions";
...
function Page() {
  ...
  // For bar chart - top 10 categories
        ...
        <div className="flex gap-3">
          ...
          {canWrite && (
            <Dialog open={addOpen} onOpenChange={setAddOpen}>
              ...
              <BudgetDialog
                year={selectedYear}
                onDone={() => { setAddOpen(false); qc.invalidateQueries({ queryKey: ["budget-vs-actual"] }); }}
              />
            </Dialog>
          )}
        </div>
        ...
        <Card>
          <CardContent className="pt-4">
            ...
            <p className={`text-2xl font-bold font-mono ${totalVariance >= 0 ? "text-green-500" : "text-red-500"}`}>
              ...
              {totalVariance >= 0 ? <TrendingDown className="inline w-5 h-5 ml-1" /> : <TrendingUp className="inline w-5 h-5 ml-1" />}
            </p>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {(rows as any[]).map((r, i) => {
                  ...
                  const pct = Number(r.utilisation_pct);
                  return (
                    <TableRow key={i}>
                      <TableCell>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex-1">
                            <Progress
                              ...
                              className="h-1.5"
                              // Red if over 100%, yellow 80-100%, green <80%
                            />
                          </div>
                          <Badge
                            variant="outline"
...
function BudgetDialog({ year, onDone }: { year: number; onDone: () => void }) {
  ...
  const m = useMutation({
    mutationFn: () => uFn({ data: { ...f, category_id: f.category_id || undefined, term: f.term || undefined } }),
    onSuccess: () => { toast.success("Budget line saved"); onDone(); },
