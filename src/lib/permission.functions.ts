...
export const editWithOverride = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: before } = await supabaseAdmin.from(data.resource).select(data.field).eq("id", data.resource_id).maybeSingle();
    const oldVal = (before as any)?.[data.field] ?? null;

    const update: any = {}; update[data.field] = data.new_value;
    const { error } = await supabaseAdmin.from(data.resource).update(update).eq("id", data.resource_id);
    if (error) throw new Error(error.message);
