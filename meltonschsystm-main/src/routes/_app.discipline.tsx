...
function Page() {
  ...
  const trend = (() => {
    const days: Record<string, { date: string; minor: number; major: number; severe: number }> = {};
    const now = new Date();
    for (let i = 29; i >= 0; i--) {
      const d = new Date(now.getTime() - i * 864e5).toISOString().slice(0, 10);
      days[d] = { date: d.slice(5), minor: 0, major: 0, severe: 0 };
    }
    for (const r of data as any[]) {
      const k = (r.incident_date ?? "").slice(0, 10);
      const slot = days[k];
      if (slot && (r.severity === "minor" || r.severity === "major" || r.severity === "severe")) {
        slot[r.severity as "minor" | "major" | "severe"]++;
      }
    }
  ...
  function exportCsv() {
    downloadCsv("discipline-incidents", data as any[], [
      { header: "Date", value: (r: any) => r.incident_date ?? "" },
      { header: "Admission", value: (r: any) => r.students?.admission_no ?? "" },
      { header: "Student", value: (r: any) => `${r.students?.first_name ?? ""} ${r.students?.last_name ?? ""}`.trim() },
      { header: "Category", value: (r: any) => r.category ?? "" },
      { header: "Severity", value: (r: any) => r.severity ?? "" },
      { header: "Description", value: (r: any) => r.description ?? "" },
      { header: "Action", value: (r: any) => r.action_taken ?? "" },
    ]);
  }
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={exportCsv} disabled={data.length === 0}><Download className="w-4 h-4 mr-2" />CSV</Button>
        ...
        <div className="h-48 mb-4">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={trend}>
              ...
              <Tooltip />
              <Bar dataKey="minor" stackId="a" fill="hsl(var(--muted-foreground))" />
              <Bar dataKey="major" fill="#f59e0b" stackId="a" />
              <Bar dataKey="severe" fill="hsl(var(--destructive))" stackId="a" />
            </BarChart>
          </ResponsiveContainer>
        </div>
