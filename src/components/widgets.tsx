  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: student } = await supabase
        .from("students")
        .select("first_name, last_name, admission_no, unique_id, gender, date_of_birth, classes(name)")
        .eq("id", (link as any).student_id)
        .maybeSingle();
      return student ? { ...student, full_name: `${student.first_name ?? ""} ${student.last_name ?? ""}`.trim() } : null;
    },
  });
  return (
    <Card>
      ...
      <CardContent>
        {isLoading ? <MiniLoader /> : data ? (
          <div className="text-sm space-y-1">
            ...
            <div className="text-muted-foreground">Admission #{(data as any).admission_no ?? (data as any).unique_id}</div>
            <div className="text-muted-foreground">Class: {(data as any).classes?.name ?? "—"}</div>
            {(data as any).gender && <div className="text-muted-foreground capitalize">Gender: {(data as any).gender}</div>}
          </div>
        ) : <EmptyState>Profile not linked yet. Contact admin.</EmptyState>}
      </CardContent>
    </Card>
  );
}
...
export function StudentUpcomingExamsWidget() {
  const { data, isLoading } = useQuery({
    queryKey: ["dashboard-student-exams"],
    queryFn: async () => {
      const today = new Date().toISOString().slice(0, 10);
      const { data } = await supabase.from("exams").select("id, name, term, year, start_date, end_date").gte("start_date", today).order("start_date").limit(3);
      return data ?? [];
    },
  });
  return (
    <Card>
      ...
      <CardContent>
        {isLoading ? <MiniLoader /> : (data as any[])?.length === 0 ? (
          <EmptyState>No upcoming exams scheduled</EmptyState>
        ) : (
          <ul className="space-y-2 text-sm">
            {(data as any[])?.map((e: any) => (
              <li key={e.id} className="flex justify-between border-b pb-1 last:border-0">
                <div>
                  <div className="font-medium">{e.name}</div>
                  <div className="text-xs text-muted-foreground">{e.term} {e.year}</div>
                </div>
                <div className="text-xs text-muted-foreground text-right">{e.start_date}</div>
              </li>
            ))}
          </ul>
        )}
...
export function StudentRecentResultsWidget() {
  const { data, isLoading } = useQuery({
    queryKey: ["dashboard-student-results"],
    queryFn: async () => {
      const { data: u } = await supabase.auth.getUser();
      const uid = u.user?.id;
      if (!uid) return [];
      ...
      const { data } = await supabase
        .from("exam_results")
        .select("score, grade, subjects(name, code), exams(name)")
        .eq("student_id", (link as any).student_id)
            ...
            {(data as any[])?.map((r: any, i: number) => (
              <li key={i} className="flex justify-between border-b pb-1 last:border-0">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{r.exams?.name}</div>
                </div>
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: links } = await supabase
        ...
        .select("students(id, first_name, last_name, admission_no, classes(name))")
