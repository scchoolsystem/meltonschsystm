  ...
  const m = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase.rpc("issue_card_assignment", {
        ...
        p_vehicle_type: needsBay ? vehicleType : null,
        ...
        <form onSubmit={e => { e.preventDefault(); m.mutate(); }} className="space-y-3">
          ...
          {needsBay && (
            <>
              <div>
                <Label>Vehicle Type</Label>
                <Select value={vehicleType} onValueChange={v => { setVehicleType(v); setF(p => ({ ...p, parking_bay_id: "" })); }}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{VEHICLE_TYPES.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            ...
            return (
              <Card key={b.id} className={borderClass}>
                <CardContent className="pt-4 space-y-1">
                  ...
                  {b.bay_type !== "general" && <Badge variant="outline" className="text-[10px]">{b.bay_type}</Badge>}
                  {assignment && (
...
const BAY_TYPES = [
  ...
  { value: "delivery", label: "Delivery / Lorries" },
];

function BayDialog({ onDone, schoolId }: { onDone: () => void; schoolId?: string }) {
  const [f, setF] = useState({ bay_code: "", zone: "", bay_type: "general" });
  const m = useMutation({
    mutationFn: async () => {
      ...
      <form onSubmit={e => { e.preventDefault(); m.mutate(); }} className="space-y-3">
        ...
        <div>
          ...
          <Select value={f.bay_type} onValueChange={v => setF(p => ({ ...p, bay_type: v }))}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>{BAY_TYPES.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent>
          </Select>
        </div>
