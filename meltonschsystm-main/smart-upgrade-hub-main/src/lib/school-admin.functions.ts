...
const SENDER_DOMAIN = "notify.smartdev.co.ke";
const FROM_DOMAIN = "smartdev.co.ke";
...
export const provisionSchoolAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    // Guard: only platform owners may provision school super-admins.
    const { data: isOwner, error: ownerErr } = await context.supabase.rpc(
      "is_platform_owner",
      { _user_id: context.userId },
    );
    if (ownerErr) throw new Error(ownerErr.message);
    if (!isOwner) throw new Error("Unauthorized: platform owner only");
    ...
    try {
      ...
      const fromName = school.name || SITE_NAME;
      const replyTo = (school as any).email || null;
