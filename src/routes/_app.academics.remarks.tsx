      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="overflow-x-auto pb-1">
          <TabsList className="inline-flex w-auto min-w-full sm:min-w-0 h-auto flex-nowrap gap-0.5 p-1">
            ...
            <TabsTrigger value="streamrank" className="whitespace-nowrap text-xs gap-1.5"><Award className="w-3.5 h-3.5" /> Stream Ranking</TabsTrigger>
          </TabsList>
        </div>
                  ...
                  {[...filtered]
                    ...
                    return (
                      <TableRow key={r.id} className="text-sm hover:bg-muted/40 transition-colors">
                        <TableCell className="font-bold text-muted-foreground text-center w-8">{medal}</TableCell>
        ...
        {/* ─── STREAM RANKING TAB ───────────────────────────────────────── */}
        <TabsContent value="streamrank" className="mt-4 space-y-4">
          <StreamRankingPanel filtered={filtered} />
        </TabsContent>
      </Tabs>
    </div>
  );
...
// ── Stream Ranking Panel ──────────────────────────────────────────────────────
// Groups students by class+stream, computes per-student totals, ranks within each group.

function StreamRankingPanel({ filtered }: { filtered: any[] }) {
  const [selectedStream, setSelectedStream] = useState<string>("all");

  // Build per-student aggregates from the flat results rows
  const studentMap = useMemo(() => {
    const m = new Map<string, {
      id: string; name: string; admNo: string;
      className: string; stream: string; streamKey: string;
      total: number; count: number;
    }>();
    for (const r of filtered) {
      const sid = r.student_id ?? r.students?.id ?? r.id;
      if (!sid) continue;
      const existing = m.get(sid);
      const className = r.students?.classes?.name ?? "—";
      const stream = r.students?.classes?.stream ?? "";
      const streamKey = `${className}${stream ? " " + stream : ""}`;
      if (existing) {
        ...
        m.set(sid, {
          ...
          admNo: r.students?.admission_no ?? "",
          className, stream, streamKey,
  ...
  const students = useMemo(() => Array.from(studentMap.values()), [studentMap]);

  const streamKeys = useMemo(() => {
    const set = new Set<string>();
    for (const s of students) set.add(s.streamKey);
  ...
  const streamGroups = useMemo(() => {
    ...
    // Sort each group by average descending
    for (const key of Object.keys(groups)) {
      groups[key].sort((a, b) => (b.total / b.count) - (a.total / a.count));
    }
    return groups;
  }, [students]);

  const visibleKeys = selectedStream === "all" ? streamKeys : [selectedStream];

  if (students.length === 0) {
    return (
      <div className="h-40 grid place-items-center text-sm text-muted-foreground">
        No results. Apply filters or select an exam first.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-xs text-muted-foreground font-medium">Stream:</span>
        <Button size="sm" variant={selectedStream === "all" ? "default" : "outline"}
          className="h-7 text-xs" onClick={() => setSelectedStream("all")}>All</Button>
        {streamKeys.map((k) => (
          <Button key={k} size="sm" variant={selectedStream === k ? "default" : "outline"}
            className="h-7 text-xs" onClick={() => setSelectedStream(k)}>{k}</Button>
        ))}
      </div>

      {visibleKeys.map((key) => {
        const group = streamGroups[key] ?? [];
        return (
          <Card key={key}>
            <CardHeader className="pb-2 pt-4 px-4">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                {key}
                <span className="text-sm font-normal text-muted-foreground">— {group.length} students</span>
              </CardTitle>
            </CardHeader>
                ...
                <Table>
                  ...
                  <TableBody>
                    {group.map((s, i) => {
                      ...
                      const g = fallbackGrade(avg);
                      const pos = i + 1;
                      const medal = pos === 1 ? "🥇" : pos === 2 ? "🥈" : pos === 3 ? "🥉" : pos;
                      return (
                        <TableRow key={s.id} className="text-sm hover:bg-muted/40">
                          <TableCell className="font-bold text-muted-foreground text-center">{medal}</TableCell>
                          <TableCell className="font-medium">{s.name}</TableCell>
                          <TableCell className="text-xs font-mono text-muted-foreground">{s.admNo}</TableCell>
                          <TableCell className="text-right tabular-nums">{s.total}</TableCell>
                          <TableCell className="text-right tabular-nums font-semibold">{avg}%</TableCell>
                          <TableCell>
