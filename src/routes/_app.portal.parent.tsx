  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      >
        ...
        {avgScore !== null && <StatCard icon={<Trophy className="w-4 h-4" />} label="Academic Average" value={avgScore} suffix="%" hint={`Grade ${gradeLabel(avgScore).grade}`} />}
      ...
      {totalDue > 0 && (
        <div className="flex items-center gap-3 p-4 rounded-xl border border-amber-400 bg-amber-50 dark:bg-amber-950/20">
                    ...
                    {examResults.map((r: any) => (
                      <div key={r.id} className="flex justify-between border-b py-1.5 text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-bold" style={{ color: gradeLabel(Number(r.score || 0)).color }}>{r.score}%</span>
        ...
        <PortalTabContent value="attendance">
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-4">
              ...
              <GlassCard className="text-center p-4">
                <div className="text-2xl font-bold text-red-500"><AnimatedNumber value={data.attendance.filter((a: any) => a.status === "absent").length} /></div>
                <div className="text-xs text-muted-foreground">Absent</div>
              </GlassCard>
              <GlassCard className="text-center p-4">
                <div className="text-2xl font-bold text-amber-500"><AnimatedNumber value={data.attendance.filter((a: any) => a.status === "late").length} /></div>
        ...
        <PortalTabContent value="fees">
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-4">
              ...
              <GlassCard className="text-center p-4">
                <div className="text-lg font-bold text-emerald-600">KES <AnimatedNumber value={(totalFees - totalDue) / 1000} decimals={1} suffix="k" /></div>
                    ...
                    {outstanding > 0 && (
                      <div className="flex justify-end">
                        <MpesaPayDialog
                          ...
                          defaultPhone={active?.parent_phone ?? ""}
                        />
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="border rounded-lg p-3">
                    ...
                    <div className="font-medium">{data.transport.pickup_point ?? data.transport.transport_routes?.pickup_point ?? "—"}</div>
                  </div>
                  <div className="border rounded-lg p-3">
                    <div className="text-xs text-muted-foreground">Drop-off</div>
                    <div className="font-medium">{data.transport.transport_routes?.dropoff_point ?? "—"}</div>
                  </div>
                  <div className="border rounded-lg p-3">
                    <div className="text-xs text-muted-foreground">Driver</div>
                    <div className="font-medium">{data.transport.transport_routes?.driver_name ?? "—"}</div>
                    {data.transport.transport_routes?.driver_phone && <div className="text-xs text-muted-foreground">{data.transport.transport_routes.driver_phone}</div>}
                  </div>
                  <div className="border rounded-lg p-3">
                    <div className="text-xs text-muted-foreground">Monthly fee</div>
                    <div className="font-medium">KES {Number(data.transport.transport_routes?.monthly_fee ?? 0).toLocaleString()}</div>
                  </div>
                </div>
              </div>
            )}
